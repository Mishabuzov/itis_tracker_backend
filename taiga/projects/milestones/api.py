...
import taiga.base.notifications.milestone_notifications as milestone_push
...
class MilestoneViewSet(HistoryResourceMixin, WatchedResourceMixin,
    ...
    def create(self, request, *args, **kwargs):
        ...
        first_milestone = Milestone.objects.first()
        role_map = {}
        points = first_milestone.total_points

        for id, story_point in points.most_common():
            from taiga.users.models import Role
            role_name = Role.objects.filter(id=id).first()
            role_map[role_name] = story_point

        stories = UserStory.objects.filter()
        for userStory in stories:
            for rolePoint in userStory.get_role_points().get_queryset():
                role_map[rolePoint.role.name] = rolePoint.points
        # for role, point in role_map.items():
        ...
        with advisory_lock("milestone-creation-{}".format(project_id)):
            ...
            if response.status_code == 201:
                # Here create push
                new_sprint_id = response.data.get("id")
                start_date = time.strptime(request.DATA.get("estimated_start"), "%Y-%m-%d")
                finish_date = time.strptime(request.DATA.get("estimated_finish"), "%Y-%m-%d")

                milestone_push.enable_milestone_push(project_id, new_sprint_id, start_date, finish_date)
            return response
