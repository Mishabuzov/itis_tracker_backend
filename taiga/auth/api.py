...
class AuthViewSet(viewsets.ViewSet):
    ...
    def _public_register(self, request):
        ...
        try:
            ...
            user = public_register(request.DATA.get("push_id"), **data)
    ...
    def _private_register(self, request):
        ...
        user = private_register_for_new_user(request.DATA.get("push_id"), **data)
    ...
    def create(self, request, **kwargs):
        ...
        if login_type in auth_plugins:
            ...
            user_id = data['id']
            if invitation_token:
                accept_invitation_by_existing_user(invitation_token, user_id)
            self.change_user_push_id(user_id, request.DATA.get("push_id", None))
            return response.Ok(data)

        raise exc.BadRequest(_("invalid login type"))

    def change_user_push_id(self, user_id, push_id):
        if push_id is not None:
            from taiga.users.models import User
            # change user's push_id after authorization
